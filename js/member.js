...
function refreshList(){
  window.location = "pages.php?page=member" ;
}
function Baru(){
  window.location = "pages.php?page=member&action=baru" ;
}
function Batal(){
  window.location = "pages.php?page=member" ;
}
function Edit(){
  var errMsg = getJumlahChecked("member");
  if(errMsg == ''){
    $.ajax({
      type:'POST',
      ...
      success: function(data) {
        var resp = eval('(' + data + ')');
        if(resp.err==''){
          window.location = "pages.php?page=member&action=edit&idEdit="+resp.content.idEdit;
        }else{
          errorAlert(resp.err);
        }
      }
